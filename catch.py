...
ERROR_DB = {
    ...
    "ZeroDivisionError": ("Деление на ноль", "Проверь делитель перед операцией или …"),
    "FileNotFoundError": ("Файл не найден", "Убедись, что путь к файлу верный и фа…"),
    "PermissionError": ("Нет прав доступа", "Запусти скрипт с sudo или проверь chm…"),
    "KeyError": ("Ключ не найден", "Используй dict.get('key') вместо прям…"),
    "IndexError": ("Индекс за пределами", "Проверь длину списка перед обращением…"),
    "TypeError": ("Ошибка типа данных", "Проверь типы переменных через type()."),
    "ValueError": ("Неверное значение", "Убедись, что входные данные соответст…"),
    "AttributeError": ("Атрибут не найден", "Проверь наличие метода/поля у объекта…"),
    "ImportError": ("Ошибка импорта", "Установи модуль: pip install <module_…"),
    "ModuleNotFoundError": ("Модуль не найден", "Установи модуль: pip install <module_…"),
    "SyntaxError": ("Синтаксическая ошибка", "Проверь скобки, двоеточия и отступы."),
    "IndentationError": ("Ошибка отступов", "Используй либо только Tab, либо тольк…"),
    ...
    "MemoryError": ("Нехватка памяти", "Оптимизируй использование памяти или …"),
    "OverflowError": ("Переполнение числа", "Используй библиотеку decimal для боль…"),
    "StopIteration": ("Итератор исчерпан", "Используй цикл for вместо ручного nex…"),
    ...
    "OSError": ("Системная ошибка", "Проверь права доступа и наличие ресур…"),
    "IOError": ("Ошибка ввода/вывода", "Проверь диск и файловую систему."),
    "BlockingIOError": ("Блокировка IO", "Используй асинхронные операции или та…"),
    ...
    "FileExistsError": ("Файл уже существует", "Используй mode='w' или проверь os.pat…"),
    ...
    "ConnectionResetError": ("Соединение сброшено", "Сервер разорвал связь. Попробуй recon…"),
    ...
    "SSLError": ("Ошибка SSL", "Обнови сертификаты или проверь дату с…"),
    ...
    "JSONDecodeError": ("Ошибка JSON", "Проверь валидность JSON строки через …"),
    "UnicodeDecodeError": ("Ошибка кодировки", "Укажи encoding='utf-8' при открытии ф…"),
    "UnicodeEncodeError": ("Ошибка записи", "Проверь поддерживаемые символы кодиро…"),
    "CSVError": ("Ошибка CSV", "Проверь разделители и экранирование."),
    "PickleError": ("Ошибка сериализации", "Убедись, что объект можно пиклить."),

    "BrokenProcessPool": ("Воркер крашнулся", "Проверь память и отсутствие глобально…"),
    ...
    "ValidationError": ("Ошибка валидации", "Проверь входные данные на соответстви…"),
    ...
    "IntegrityError": ("Нарушение целостности", "Проверь уникальные ключи и внешние св…"),
    ...
    "SessionPasswordNeeded": ("Нужен 2FA", "Укажи пароль двухфакторной аутентифик…"),
    "PhoneCodeInvalidError": ("Неверный код", "Проверь код из SMS/Telegram."),
    ...
    "ChannelsTooMuchError": ("Много каналов", "Выступи из части каналов перед вступл…"),
    "UsersTooMuchError": ("Много пользователей", "Лимит добавления участников превышен."),
    "BroadcastPublicVotersForbidden": ("Голосование публично", "Нельзя создавать публичные опросы здесь."),
    ...
    "ChatSendInlineForbidden": ("Запрет Inline", "В чате запрещено использовать inline …"),
    ...
    "ChatSendStickersForbidden": ("Запрет стикеров", "В чате запрещено отправлять стикеры."),
    ...
    "DataInvalid": ("Неверные данные", "Проверь формат передаваемых данных."),
    "DcIdInvalid": ("Неверный DC ID", "Проверь идентификатор дата-центра."),
    "DialogFilterExcludeTooMany": ("Много исключений", "Уменьши количество исключений в фильтре."),
...
def _show_welcome():
    ...
    if Colors.is_terminal():
        print(f"\n{Colors.GREEN}{Colors.BOLD}  Catch Error Handler v8.0 {Colors.RESET}")
             print(f"{Colors.CYAN}   Mode: {'ON' if _enabled else 'OFF'} | Log: {LOG_FILE} | Py {sys.version.split()[0]}{Colors.RESET}")
        print(f"{Colors.YELLOW}    Import FIRST!{Colors.RESET}\n")
    else:
        print(f"\n  Catch Error Handler v8.0 ")
